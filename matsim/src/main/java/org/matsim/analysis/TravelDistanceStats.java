...
public class TravelDistanceStats {
	...
	public void addIteration(int iteration, Map<Id<Person>, Plan> map) {
		...
		this.threads = null;
		log.info("-- average of the average leg distance per plan (executed plans only): " + (sumAvgPlanLegTravelDistanceExecuted ));//KAMIJO
		...
		try {
			...
			this.out.write(iteration + "\t" + (sumAvgPlanLegTravelDistanceExecuted) + "\t" + "\n");//KAMIJO
			this.out.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		if (this.history != null) {
			...
			//this.history[index] = (sumAvgPlanLegTravelDistanceExecuted / nofLegTravelDistanceExecuted);//KAMIJO
			this.history[index] = (sumAvgPlanLegTravelDistanceExecuted);//KAMIJO
	...
	private class StatsCalculator implements Runnable {
		...
		private double getAvgLegTravelDistance(final Plan plan) {
			...
			for (PlanElement pe : plan.getPlanElements()) {
				if (pe instanceof Leg) {
					...
					if (!Double.isNaN(distance)) {
						if(leg.getMode().equals("pav")||leg.getMode().equals("car")) {  // TODO KAMIJO
			...
			if (numberOfLegs>0) {
				return planTravelDistance;///numberOfLegs;// TODO KAMIJO
			}
			return 0.0;
		}
	}
