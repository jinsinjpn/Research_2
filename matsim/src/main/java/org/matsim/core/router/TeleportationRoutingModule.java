...
public class TeleportationRoutingModule implements RoutingModule {
	...
	/* package */ double routeLeg(Person person, Leg leg, Activity fromAct, Activity toAct, double depTime) {
		...
		int travTime = (int) (estimatedNetworkDistance / this.networkTravelSpeed);//KAMIJO//YOJIN teleport（徒歩、自転車）に余計な時間をかけるとこ　（使ってはいない）
		/*
		int travTime = 0;
		if(leg.getMode()=="bike") {
			travTime = (int) (estimatedNetworkDistance / this.networkTravelSpeed)+30;
		}
		else {
			travTime = (int) (estimatedNetworkDistance / this.networkTravelSpeed);//KAMIJO
		}
		*/
