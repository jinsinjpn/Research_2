...
public class AStarEuclidean extends Dijkstra {
	 ...
	 * @param overdoFactor The factor which is multiplied with（にかける） the output of the A* heuristic function. The higher
	 * the overdo factor the greedier（貪欲な） the router, i.e. it visits less nodes during routing and is thus faster, but
	 * for an overdo factor > 1, it is not guaranteed that the router returns the least-cost paths. Rather（むしろ） it tends （〜しがちになる）
