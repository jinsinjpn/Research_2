...
public final class ScoringFunctionsForPopulation implements BasicEventHandler, EventsToLegs.LegHandler, EventsToActivities.ActivityHandler {
	...
	private void init() {
		for (Person person : population.getPersons().values()) {
			...
			//System.out.println(data.getScore());//KAMIJO
...
//				}
	...
	synchronized public void handleLeg(PersonExperiencedLeg o) {
		...
		if (scoringFunction != null) {
			...
			*/
			...
			//partialScoresForAgent.add(score);//KAMIJO
			//System.out.println(agentId);//KAMIJO//これらをつけるとその移動のスコアまでの合算値が表示される
			//System.out.println(leg);//
			//System.out.println(scoringFunction.getScore());//
		}
	}
	...
	synchronized public void handleActivity(PersonExperiencedActivity o) {
		...
		if (scoringFunction != null) {
			...
			TDoubleCollection partialScoresForAgent = partialScores.get(agentId);
			partialScoresForAgent.add(scoringFunction.getScore());
			//System.out.println(agentId);//KAMIJO//これをつけるとそのアクティビティのスコアまでの合算値が表示される
	...
	public ScoringFunction getScoringFunctionForAgent(final Id<Person> agentId) {
		//System.out.println(this.agentScorers.get(agentId).getScore());//KAMIJO
	...
	public void finishScoringFunctions() {
		...
		for (Entry<Id<Person>, TDoubleCollection> entry : this.partialScores.entrySet()) {
			...
			//entry.getValue().add(this.getScoringFunctionForAgent(entry.getKey()).getScore()-1000);//KAMIJO
			entry.getValue().add(this.getScoringFunctionForAgent(entry.getKey()).getScore());

			//System.out.println(entry.getValue());//KAMIJO
		}
	}
