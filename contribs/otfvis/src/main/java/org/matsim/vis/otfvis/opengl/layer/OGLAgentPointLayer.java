...
public class OGLAgentPointLayer extends OTFGLAbstractDrawable implements SceneLayer {
	...
	public void addAgent(AgentSnapshotInfo agInfo) {
		...
		case taxicab:
			...
			break;
		case drtcab://KAMIJO
			color = drtcabColoringScheme(agInfo) ;
			break;
		default:
			color = standardColoringScheme(agInfo);
			break;
	...
	private static Color taxicabColoringScheme(AgentSnapshotInfo agInfo) {
        ...
        else if (agInfo.getId().toString().startsWith("drt")) {//KAMIJO
            if (agInfo.getAgentState() == AgentState.PERSON_DRIVING_CAR) {
                return Color.cyan ;
            }
            else {
	...
	private static Color drtcabColoringScheme(AgentSnapshotInfo agInfo) {
        // ===============DRT COLOURING===============//KAMIJO
        if (agInfo.getId().toString().startsWith("drt")) {//KAMIJO
            if (agInfo.getAgentState() == AgentState.PERSON_DRIVING_CAR) {
                return Color.cyan ;
            }
            else {
        ...
        /*
        else if (agInfo.getId().toString().startsWith("walk")) {//KAMIJO
            if (agInfo.getAgentState() == AgentState.PERSON_OTHER_MODE) {
        ...
        else if (agInfo.getId().toString().startsWith("bike")) {//KAMIJO
            if (agInfo.getAgentState() == AgentState.PERSON_OTHER_MODE) {
                return Color.red ;
            }
            else {
