...
public class DrtAnalysisControlerListener implements IterationEndsListener {
	...
	public void notifyIterationEnds(IterationEndsEvent event) {
		...
		String tripsSummarize =DynModeTripsAnalyser.summarizeTrips(trips, ",");//KAMIJO
		double directDistanceMean = DynModeTripsAnalyser.getDirectDistanceMean(trips);
		writeIterationPassengerStats(tripsSummarize+","+drtRequestAnalyzer.getRejections().size()+","+format.format(rejectionRate), event.getIteration());//KAMIJO
		double l_d= DynModeTripsAnalyser.getTotalDistance(drtPassengerStats.getVehicleDistances()) / (trips.size()*directDistanceMean);
		String vehStats = DynModeTripsAnalyser.summarizeVehicles(drtPassengerStats.getVehicleDistances(), ",")+","+format.format(l_d);//KAMIJO
	...
	private void writeIterationPassengerStats(String summarizeTrips, int it) {
		...
		try {
			...
			bw.newLine();
			bw.write(runId+","+ it + "," + summarizeTrips);//KAMIJO
			bw.flush();
			bw.close();
		} catch (IOException e) {
	...
	private void writeIterationVehicleStats(String summarizeVehicles, int it) {
		...
		try {
			...
			bw.newLine();
			bw.write(runId+","+ it + "," + summarizeVehicles);//KAMIJO
			bw.flush();
			bw.close();
		} catch (IOException e) {
			// TODO Auto-generated catch block
