		...
		for (Entry<Id<Vehicle>, int[]> e : this.vehicleOccupancy.entrySet()) {
			...
			try {
				...
				for (int i = 0; i < e.getValue().length; i++) {
					bw.newLine();
					bw.write(startTime + i + "," + e.getValue()[i]);//KAMIJO
				}
				bw.flush();
				bw.close();
	...
	public void calcAndWriteFleetStats(String statsFileName, boolean writeDetailedStats) {
		...
		try {
			...
			for (int i = 0; i < bins; i++) {
				...
				bw.write(time + "," + format.format(mean) + "," + format.format(median) + "," + min + "," + max);//KAMIJO
				for (int ii = 0; ii <= maxcap + 1; ii++) {
					bw.write("," + occupancyOverTime[i][ii]);//KAMIJO
				}
			}
		...
		try {
			bw.write("time,idle,emptyDrive");//KAMIJO
			for (int i = 1; i <= maxcap; i++) {
				bw.write("," + i + "_pax");//KAMIJO
			}
			for (int i = 0; i < occupancyOverTimeAverage.length; i++) {
				...
				for (int ii = 0; ii <= maxcap + 1; ii++) {
					bw.write("," + occupancyOverTimeAverage[i][ii]);//KAMIJO
				}
			}
