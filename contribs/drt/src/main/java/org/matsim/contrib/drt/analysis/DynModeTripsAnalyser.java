	...
	public static void analyseDetours(Network network, List<DynModeTrip> trips, double beelineDistanceFactor,
		...
		for (DynModeTrip trip : trips) {
			...
			detours.add(trip.getTravelDistance() + "," + distance + "," + distanceDetour + ","
					+ (trip.getArrivalTime() - trip.getDepartureTime()) + "," + time + "," + timeDetour);//KAMIJO
		}
	...
	public static void analyseWaitTimes(String fileName, List<DynModeTrip> trips, int binsize_s) {
		...
		try {
			...
			for (Entry<Double, List<DynModeTrip>> e : splitTrips.entrySet()) {
				...
				bw.newLine();
				bw.write(Time.writeTime(e.getKey()) + "," + rides + "," + format.format(averageWait) + ","//KAMIJO
	...
	public static void writeVehicleDistances(Map<Id<Vehicle>, double[]> vehicleDistances, String iterationFilename) {
		...
		try {
			...
			for (Entry<Id<Vehicle>, double[]> e : vehicleDistances.entrySet()) {
				...
				bw.newLine();
				bw.write(e.getKey().toString() + "," + format.format(drivenDistance) + "," + format.format(occDistance)//KAMIJO
